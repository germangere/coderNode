...
export const getCart = async (req, res) => {
    const exist = await DB.listarByEmail(req.session.email);
    res.json(exist ?? {});
}
...
export const delItemFromCart = async (req, res) => {
    const result = await deleteItem(req.session.email, req.params.id);
    res.json(result);
}

export const deleteCart = async (req, res) => {
    const cart = await DB.listarByEmail(req.session.email);
